...
The process returns exit code 0 only when every selected subject passes atlas region-set, ROI-count, `aseg`, serialized-output, and optional requested QC/export checks. A nonzero exit code does not mean the cohort tables are empty: available current-run data from `PARTIAL` and `FAILED` subjects are included, with `status` and `errors` in both long and wide tables. `NOT_RUN` and previous-run data are excluded. Every run has a unique `run_id`.

Aggregation preserves actual region names without normalization. Wide columns are the union of accepted names in the selected cohort, with empty cells for unavailable values; `&` and `_and_` spellings remain separate columns. `region_differences.tsv` lists missing expected names, unexpected names, and names present elsewhere in the cohort but absent from that subject. Its `reference` distinguishes a known atlas definition from a cohort-only comparison; a name difference is not necessarily an anatomical difference. A region absent from the entire cohort appears in the expected-name report when its definition is available, but does not create an unobserved wide column.

Checksums and table structure are checked independently for each subject file. A missing, unreadable, checksum-mismatched or invalid-header file is excluded without removing other files or subjects. Malformed rows, invalid numbers and ambiguous duplicate keys are excluded locally; all copies of a duplicate key are excluded rather than choosing one. For aseg, both segment IDs and structure names must be unambiguous; global measure and metric names must also be unambiguous. The reasons are recorded in `errors`, and original per-subject TSVs are retained. Cohort inclusion does not imply that reconstruction, atlas validation or QC passed.
...
Accepted rows are staged in a run-owned temporary directory so long and wide tables use the same records without modifying per-subject TSVs. Temporary tables are removed when aggregation exits. Only one subject's tables and cohort column names are held in memory; allow temporary disk space for the accepted TSVs. `all_features_wide.tsv` can still be large on disk, but the complete cohort matrix is not retained in RAM.
